...
## PLACEHOLDER-DATE (content grounding)

<!-- HELD: set this heading to the date the tools go LIVE on api.marcora.ai, per
     docs/mcp-change-fanout-checklist.md §0. Do not publish before then. -->
...
- **Content grounding — three new tools (`check_content_grounding`, `get_grounding_result`, `apply_grounding_fix`).** Grounding checks a document's factual claims against the team's own context library and sorts each into supported, conflict, or gap. Together the three close the agent-writing loop — draft, ground, review, apply, re-ground — without leaving the conversation. Requires the Command plan.
  ...
  - `apply_grounding_fix` applies a finding's stored recommendation by id — the agent never composes the fix itself. It serves **both** content-grounding findings and Context Intelligence health-audit recommendations, and returns one job per finding. Poll `get_generation_status` with a job's `generation_id` (an integer); the `document_updated` field there tells you whether the document actually changed or the recommendation was already covered.
